...
def make_legit_lookalike_rows(
    *,
    ...
    for idx, row in enumerate(attack_rows):
        ...
        variant["beneficiary_added_ago_s"] = int(rng.integers(5 * 86400, 30 * 86400))
...
class DeviceFanOutAttack(AttackGenerator):
    ...
    def generate(
        ...
        *,
        ...
        device_id = baseline.tables["devices"][min(10, len(baseline.tables["devices"]) - 1)][
            "device_id"
        ]
        ...
        merchant_ids = [
            row["party_id"] for row in baseline.tables["parties"] if row["party_type"] == "merchant"
        ]
        ...
        for card_idx, payer_id in enumerate(payer_ids):
            ...
            txn = _transaction_row(
                ...
                channel="online",
                merchant_id=merchant_id,
                mcc=rng.integers(4000, 8500),
                auth_method="cvv",
                auth_result="success",
                decision="approved",
                session_duration_s=int(20 + rng.integers(0, 60)),
                time_on_confirm_screen_s=float(rng.uniform(1.0, 3.0)),
                beneficiary_first_time=True,
                beneficiary_added_ago_s=int(rng.integers(5, 120)),
                pin_attempts=1,
                screen_share_active=False,
                call_active_during_txn=False,
                geo_matches_home=False,
                purpose_code="00",
                issuer_risk_score=0.25,
            )
            rows.append(txn)
            labels.append(
                _label_row(
        ...
        campaign = _build_campaign(
            ...
            start_time=rows[0]["timestamp"],
            end_time=rows[-1]["timestamp"],
            affected_entities=payer_ids + [device_id],
            event_count=len(rows),
            pretext="compromised_device",
            config={"device_id": device_id, "n_cards": n_cards},
        )
        for label in labels:
            label["campaign_id"] = campaign.campaign_id
    ...
    def generate(
        ...
        *,
        ...
        rxn = _transaction_row(
            ...
            auth_result="success",
            decision="approved",
            session_duration_s=int(30 + rng.integers(0, 20)),
            time_on_confirm_screen_s=float(rng.uniform(1.5, 3.0)),
            beneficiary_first_time=False,
            beneficiary_added_ago_s=int(rng.integers(200, 1000)),
            pin_attempts=1,
            screen_share_active=False,
            call_active_during_txn=False,
            geo_matches_home=True,
        ...
        device_id_recv = (
            _choose_device_for_party(baseline, receiver_id)
            or baseline.tables["devices"][0]["device_id"]
        )
        ...
        txn = _transaction_row(
            ...
            auth_result="success",
            decision="approved",
            session_duration_s=int(20 + rng.integers(0, 15)),
            time_on_confirm_screen_s=float(rng.uniform(1.2, 2.5)),
            beneficiary_first_time=True,
            beneficiary_added_ago_s=int(rng.integers(5, 60)),
            pin_attempts=1,
            screen_share_active=False,
            call_active_during_txn=False,
            geo_matches_home=True,
        ...
        campaign = _build_campaign(
            ...
            start_time=rows[0]["timestamp"],
            end_time=rows[-1]["timestamp"],
            affected_entities=[payer_id, receiver_id, beneficiary_id],
            event_count=len(rows),
            pretext="money_movement_fraud",
            config={"drain_ratio": drain_ratio},
        )
        for label in labels:
            label["campaign_id"] = campaign.campaign_id
