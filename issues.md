...
1. **P0 — Fill the deck placeholders:** `docs/deck/chakravyuh-walkthrough.pptx` has two
   ...
   directly, and capture the screenshot once a model is trained (`stage5/models/*.pkl`
   exist) and there's a display available to run `streamlit run web/app.py` against.
...
4. **P3 — stage5 artifacts (heavy, deliberately deferred):** `stage5/models/fraud_model.pkl`/
