...
6. **Single-transaction rule overrides added this session, and the families found to be structurally unfixable without one**: five families (`insider_abuse`, the `synthetic_identity_bustout` max-out phase, `credential_takeover`, `card_testing_probe`) received targeted policy overrides in `pipeline.py` after verifying each signature's false-positive rate against legitimate generation calibration constants (each documented inline at its override). Three families were investigated and found to have **no safe single-transaction fix**, independent of the mule-network case above:
