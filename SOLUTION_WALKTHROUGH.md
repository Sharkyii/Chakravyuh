...
3. **Grounded attack diversity**: 16 attack families are grounded in published fraud signatures — reference datasets (IEEE-CIS, PaySim), competition write-ups, NPCI's own UPI architecture documentation, and research papers. No raw training data is used; design is driven by structural signatures (device fingerprint reuse, receive-liquidate patterns, cross-TPAP account switching) extracted from public sources.
