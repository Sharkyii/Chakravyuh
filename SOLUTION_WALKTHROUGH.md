...
| Device Fan-Out | One device/fingerprint, 4-6 distinct cards in 2h | Card | Device-centric clustering (well documented fraud signal) |
| Balance Drain Exit | Receive large transfer, liquidate 85-95% in 5min | UPI | Rapid receiver balance collapse (account-takeover pattern) |
...
**Grounding notes:**
- Device fan-out: a well documented top-tier predictive signal in fraud detection research
- Balance drain exit: a well documented account-takeover mechanism (transfer in, then rapid cash-out)
...
The items below are the highest-leverage next steps for building on this foundation - each one found by the system's own instrumentation, scoped concretely, not left as an open-ended TODO.

1. **Sharper lookalikes is the next concrete improvement** (currently 2.12x vs. 1.0x target): Lookalikes currently receive only one shape-matched transaction but lack synthetic behavioral pre-history (e.g., a pattern of prior transfers before the fraud attempt). Generating a short sequence of synthetic pre-history transactions for each lookalike would close this gap - medium effort, and the clearest next step for hardening the hardest negative class.
2. **Analyst feedback loop - done**: `FeedbackStore` and the LLM-backed analyst engine are wired into retraining end-to-end. The analyst-feedback UI calls `/api/analyst/trigger-retrain`, which invokes `stage5.training.feedback_retrain_orchestrator.run_retrain()` - it loads real analyst verdicts, retrains, and only promotes the new model if it doesn't regress PR-AUC past a small tolerance (`MAX_PR_AUC_REGRESSION`), so a bad batch of feedback can't silently degrade the deployed model.
...
6. **Single-transaction rule overrides added this session, and the families that need cross-transaction context to go further**: eight families (`insider_abuse`, `credential_takeover`, `card_testing_probe`, and - via one shared widened rule - `synthetic_identity_bustout`'s max-out phase, `synthetic_merchant`, `transaction_laundering`, `subthreshold_fragmentation`, `agentic_injection`) received targeted policy overrides in `pipeline.py` after verifying each signature's false-positive rate against legitimate generation calibration constants (each documented inline at its override). Three families were investigated and found to have **no safe single-transaction fix**, independent of the mule-network case above:
   ...
   - `stealth_mandate`: `auth_method="mandate_no_afa"` on a small amount is exactly how a legitimate low-value recurring UPI Autopay mandate looks; the only real signal is the amount escalating across a *sequence* of charges over weeks, which a single-transaction check cannot see.
   - `device_fan_out` and `balance_drain_exit`: both require genuine cross-transaction context (a shared `device_id` linked to multiple distinct payer accounts; a drain amount evaluated as a ratio of a specific prior received transaction) that isn't representable in `ALL_FEATURES` at all for `device_fan_out`, and isn't computable without the paired transaction for `balance_drain_exit`'s drain step. `balance_drain_exit`'s *receive* step is partially covered by the beneficiary-age-contradiction rule above when the add timestamp falls under 300s.

   All three point to the same next architectural layer as item 4 above: a real-time feature store (cross-account device linkage, per-account recent-transaction lookups) or campaign-level sequence modeling - not achievable via a stateless single-transaction rule, and correctly scoped as the next build rather than squeezed into this one.

7. **API hardening is progressing**: probing our own live demo API surfaced a real, verified model-extraction exposure - no rate limiting (10 sequential calls all succeeded unthrottled), full floating-point-precision `fraud_probability` returned on every response (e.g. `0.011766709387302399`), and complete per-feature SHAP contributions returned on every call. Together these let an attacker binary-search the decision boundary or approximately reconstruct the model's feature weights via query access alone (the classic "stealing ML models via prediction APIs" pattern), with SHAP output making this considerably easier than probability access alone. One related bug found during this probe - `CORSMiddleware` configured with `allow_origins=["*"]` and `allow_credentials=True`, which per the CORS spec forces the middleware to reflect the caller's `Origin` verbatim instead of a literal `*` - was fixed (`allow_credentials=False`), since the API's session scoping already uses a client-supplied `X-Session-Id` header, not cookies or `Authorization`, so no browser credential is ever legitimately sent. **Rate limiting has since been added** (`web/api.py`'s `enforce_rate_limit` - an in-process sliding window keyed on Cloudflare's `CF-Connecting-IP` header, default 30 requests/60s, scoped to `/api/analyze` only so read-only/budget-gated endpoints aren't affected; 6 unit tests in `tests/web/test_api_rate_limit.py`). Its own honestly-stated limitation: in-process state doesn't survive a container restart or share across multiple container instances, so it stops scripted single-source scraping at this deployment's current scale (one container, `sleepAfter=10m`) but isn't a substitute for a shared store (Redis, Cloudflare Rate Limiting Rules) at real production scale - the next hardening step. Precision and SHAP completeness remain deliberately undefended: full SHAP output is the entire point of the public explainability demo, and response quantization would blunt that for the same users it's meant to inform, not just attackers - a tiered-access layer (full detail for the demo UI, reduced detail for anonymous callers) is the correct fix and is scoped as future work.

8. **Consent-UI dark-pattern detection is a scoped future pipeline stage** (identified during research, not yet built): distinct from `stealth_mandate` (whose signal is amount escalation across a charge sequence), this is a GenAI-era variant where the fraud signal lives in the *consent UI itself* - an LLM-generated renewal/re-authorization prompt worded to obscure that a recurring mandate is being extended or its cap raised, rather than in any transaction-table field. This wasn't built as a trained family because doing so honestly would require either (a) real captured UI copy/screenshots to derive a genuine "disclosure clarity" signal, which this project doesn't have and won't fabricate a number for, or (b) inventing a synthetic `consent_disclosure_score` field with no grounding in any published dataset or documented mechanism - unlike all 16 trained families, which trace to a real structural signature or reference-dataset stat. Flagged here as a real, GenAI-relevant extension rather than shipped as a weakly-grounded 17th family: the schema and detector both operate on transaction/behavioral data, and consent-UI text is a fundamentally different data modality that would need its own pipeline (e.g. an LLM-as-judge scoring mandate-renewal prompt text for dark-pattern language) layered on top - the natural next stage once that data exists.
...
3. **Grounded attack diversity**: 16 attack families are grounded in published fraud signatures - well documented fraud mechanisms, NPCI's own UPI architecture documentation, and research papers. No raw training data is used; design is driven by structural signatures (device fingerprint reuse, receive-liquidate patterns, cross-TPAP account switching) extracted from public sources.
...
**16 distinct attack typologies** (13 base + 3 grounded-in-reference-data families), each with 4 curriculum difficulty levels (60 attack variants per family). Coverage spans:
...
- Cross-app (`tpap_account_switch`: a compromised UPI handle drained across 3-4 of its linked bank accounts via different TPAP apps in a tight window - grounded in NPCI's UPI interoperability architecture, where one VPA can route through several apps to several linked accounts. This is invisible to any single bank's or app's own fraud system, since no one party in the chain sees more than its own slice of the pattern)
...
- The `FeedbackStore` infrastructure already collects analyst labels and feeds them into real retraining via `/api/analyst/trigger-retrain` - no manual wiring step needed at deployment time.
...
│   ├── generate_training_data.py      # Synthetic data gen (16 families)
